...
const PurchaseDetails = () => {
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      try {
        const purchaseResponse = await axios.get(`http://localhost:5000/api/admins/purchases/${id}`);
        ...
        if (matchingSubscription) {
          const paymentResponse = await axios.get(`http://localhost:5000/api/admins/payments/${matchingSubscription.payment_id}`);
